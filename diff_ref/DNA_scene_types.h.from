...
enum eSceneEEVEE_Flag : int {
  ...
  /* Goo/legacy EEVEE only: set by pre-5.2 files that used the built-in Bloom (removed in
   * EEVEE-Next). Consumed and cleared by versioning (converted to a compositor Glare node). */
...
struct SceneEEVEE {
  ...
  /* Goo/legacy EEVEE built-in Bloom settings, only read back from pre-5.2 files (the feature was
   * removed in EEVEE-Next). Kept for versioning: converted to a compositor Glare (Bloom) node on
   * load, see `do_version_goo_bloom_to_glare`. */
  DNA_DEPRECATED float bloom_color[3] = {1.0f, 1.0f, 1.0f};
  DNA_DEPRECATED float bloom_threshold = 0.8f;
  DNA_DEPRECATED float bloom_knee = 0.5f;
  DNA_DEPRECATED float bloom_intensity = 0.05f;
  DNA_DEPRECATED float bloom_radius = 6.5f;
  DNA_DEPRECATED float bloom_clamp = 0.0f;
};
